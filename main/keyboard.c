...
#define INITIAL_REPEAT_DELAY_MS  800
...
static esp_err_t adv_tca8418_read_reg(uint8_t reg, uint8_t *data) {
    return i2c_master_transmit_receive(
        g_tca8418_dev,
        &reg,
        1,
...
static void adv_keyboard_init(void) {
...
static void org_keyboard_init(void)
{
    gpio_config_t out_conf = {
        .pin_bit_mask =
            ((1ULL << ORG_GPIO_74HC138_A0) |
             (1ULL << ORG_GPIO_74HC138_A1) |
             (1ULL << ORG_GPIO_74HC138_A2)),
...
static char debounce(char current)
{
    ...
    if ((now - first_press_time) >= INITIAL_REPEAT_DELAY_MS) {

        if ((now - last_repeat_time) >= REPEAT_INTERVAL_MS) {
...
char keyboard_get_key(void)
{
        ...
        for (int r = 0; r < ADV_NUM_ROWS; r++) {
            ...
            for (int c = 0; c < ADV_NUM_COLS; c++) {
                ...
                if (adv_key_map_state[r][c] == 1) {
                    ...
                    return debounce(adv_key_map[r][c]);
                }
            }
        }
    }
    else
    {
        ...
        for (int row = 0; row < 8; row++) {
			...
			gpio_set_level(ORG_GPIO_74HC138_A0, row & 1);
			gpio_set_level(ORG_GPIO_74HC138_A1, (row >> 1) & 1);
			gpio_set_level(ORG_GPIO_74HC138_A2, (row >> 2) & 1);

			esp_rom_delay_us(1);

			for (int col = 0; col < ORG_NUM_COLUMNS; col++) {
				if (!gpio_get_level(column_gpios[col])) {
					return debounce(org_keyboard_map[row][col]);
				}
			}
		}
