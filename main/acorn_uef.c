...
// 1200 Baud
#define HALF_BSE_US_1200	416
#define HALF_DBL_US_1200	208

static uint32_t HALF_BSE_US = 0;
static uint32_t HALF_DBL_US = 0;
...
static IRAM_ATTR void play_pulses(int half_period, int count) {
	...
	if (test_mode == false && acorn_player_user_tape_status) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (waveform_phase == 0) {
				GPIO.out_w1tc = AUDIO_OUT_PIN_MASK; // low
			}
			else {
				GPIO.out_w1ts = AUDIO_OUT_PIN_MASK; // high
			}
...
static IRAM_ATTR void play_bit(uint8_t bit) {
	if (bit) {
		play_pulses(HALF_DBL_US, 2);
	} else {
		play_pulses(HALF_BSE_US, 1);
	}
}

static IRAM_ATTR void play_leader_cycles(uint32_t cycles) {
	// double the leader cycles
	play_pulses(HALF_DBL_US, cycles * 2);
}
...
static IRAM_ATTR void chunk_0113(uint32_t len) {  // change of base frequency
	...
	if (new_freq > 0.0f) {
		HALF_DBL_US = (uint32_t)(1000000.0f / (4.0f * new_freq));
		HALF_BSE_US = HALF_DBL_US * 2;
	}
...
static IRAM_ATTR void chunk_0114(uint32_t len) { // security cycles
	...
	if (cycles_left == 1) {
		...
		if (last_flag == 'P') {
			...
			esp_rom_delay_us(bit ? HALF_DBL_US : HALF_BSE_US);
		}
		else {
...
static IRAM_ATTR void chunk_0115(uint32_t len) { // phase change 
	...
	if (phase_deg >= 90 && phase_deg <= 270) {
		waveform_phase = 1; // normal 180 degrees
	} else {
...
static void play(void) {
	...
	if (acorn_player_pos != 0) {
		...
		// tape start
		GPIO.out_w1tc = AUDIO_OUT_PIN_MASK; // low
		esp_rom_delay_us(HALF_BSE_US_1200);
		HALF_BSE_US = HALF_BSE_US_1200;
		HALF_DBL_US = HALF_DBL_US_1200;
		waveform_phase = 1; // normal 180 degrees
