...
void display_progress(void) {
	...
	if (commodore_player_file_valid) {
		...
		if (commodore_use_motor) {
			graphic_draw_status_indicator("Motor", gpio_get_level(COMMODORE_MOTOR_PIN), pos_x, pos_y, INDICATOR_MOTOR_COLOR, INDICATOR_OFF_COLOR, true);
		}
		else {
			graphic_draw_status_indicator("Motor", false, pos_x, pos_y, INDICATOR_OFF_COLOR, INDICATOR_OFF_COLOR, true);
		}
